...
Artnet.prototype.connect = function (host, port, universe, refresh, iface) {
    ...
    if (iface) {
        this.socket.bind(this.port, iface);
    }
...
Artnet.prototype.set = function (channel, value, callback) {
    ...
    if (length > 512) {
        this.data.splice(0, 512);
    } else if (length < 512) {
        this.data.concat(Array.apply(null, new Array(512 - length)).map(Number.prototype.valueOf, 0));
    }
